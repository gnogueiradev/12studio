...
- **colors** — name (**único global**, `COLLATE NOCASE`), hex_color, image?, is_active. Sem `material_id` e sem preço: quem custa dinheiro é a bobine
- **color_material** — pivô N:N (`color_id`, `material_id`, PK composta). Em que filamentos é que cada cor existe — há rosa em PLA e em Matte, não há rosa em Silk. É o que torna a matriz de variantes uma **intersecção** e não um produto cartesiano, e o que impede vender combinações inexistentes
  - Já foi 1:N (`colors.material_id`), e o erro não era ligar cor a material: era obrigar a uma linha "Rosa" por cada bobine. As migrações `000050→000070` fundiram as duplicadas e cortaram a FK; a `2026_08_16_000010` trouxe a relação de volta na cardinalidade certa
  - Nasce **vazia**: o catálogo real declara-se à mão. Uma cor sem pares é um estado legítimo (`Color::state() === 'no_material'`) — não gera variantes, mas também não esconde as que já existem. *Não ter declarado* não é *ter declarado que não existe*
